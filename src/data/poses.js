// Yoga pose seed data with target angles for AI comparison
// These are the ideal joint angles for each pose

export const YOGA_POSES = [
  {
    id: 'mountain_pose',
    name: 'Mountain Pose',
    sanskritName: 'Tadasana',
    level: 'Beginner',
    duration: 60,
    description: 'The foundation of all standing poses. Mountain Pose improves posture, balance, and calm focus.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1506126613408-eca07ce68773?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4',
    instructions: [
      'Stand with your feet together, heels slightly apart',
      'Spread your toes and distribute weight evenly',
      'Engage your thighs and lift your kneecaps',
      'Lengthen your tailbone toward the floor',
      'Roll your shoulders back and down',
      'Extend arms alongside your body, palms facing forward',
      'Hold your head level, chin parallel to the floor',
      'Breathe deeply and hold for 30-60 seconds',
    ],
    targetAngles: {
      leftElbow: 170,
      rightElbow: 170,
      leftKnee: 175,
      rightKnee: 175,
      leftShoulder: 10,
      rightShoulder: 10,
      leftHip: 175,
      rightHip: 175,
    },
    jointWeights: {
      leftElbow: 0.3, rightElbow: 0.3, leftKnee: 0.7, rightKnee: 0.7,
      leftShoulder: 0.5, rightShoulder: 0.5, leftHip: 0.8, rightHip: 0.8,
    },
    angleTolerance: {
      leftElbow: 15, rightElbow: 15, leftKnee: 15, rightKnee: 15,
      leftShoulder: 20, rightShoulder: 20, leftHip: 15, rightHip: 15,
    },
    cameraHint: 'front',
    cameraHintText: 'Place your phone standing up, facing you directly',
    setupInstructions: [
      'Stand tall with your feet together',
      'Let your arms hang naturally by your sides',
      'Roll your shoulders back and down',
      'Keep your spine straight and head level',
      'Breathe deeply and hold steady',
    ],
    benefits: ['Improves posture', 'Strengthens thighs', 'Reduces flat feet'],
  },
  {
    id: 'warrior_one',
    name: 'Warrior I',
    sanskritName: 'Virabhadrasana I',
    level: 'Beginner',
    duration: 45,
    description: 'A powerful standing pose that builds strength in the legs while opening the chest and shoulders.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ElephantsDream.mp4',
    instructions: [
      'Start in Mountain Pose',
      'Step your left foot back 3-4 feet',
      'Turn your left foot out 45 degrees',
      'Bend your right knee over right ankle to 90 degrees',
      'Raise both arms overhead, palms facing each other',
      'Square your hips toward the front',
      'Lift your chest and gaze upward',
      'Hold for 30-45 seconds, then switch sides',
    ],
    targetAngles: {
      leftElbow: 170,
      rightElbow: 170,
      leftKnee: 175, // FIXED: back knee should be straight
      rightKnee: 90,
      leftShoulder: 170,
      rightShoulder: 170,
      leftHip: 150,
      rightHip: 90,
    },
    jointWeights: {
      leftElbow: 0.3, rightElbow: 0.3, leftKnee: 0.6, rightKnee: 1.0,
      leftShoulder: 0.8, rightShoulder: 0.8, leftHip: 0.7, rightHip: 0.9,
    },
    angleTolerance: {
      leftElbow: 20, rightElbow: 20, leftKnee: 15, rightKnee: 12,
      leftShoulder: 15, rightShoulder: 15, leftHip: 20, rightHip: 15,
    },
    cameraHint: 'front',
    cameraHintText: 'Place your phone standing up, facing you directly',
    setupInstructions: [
      'Step your left foot back about 3 feet',
      'Bend your right knee to 90 degrees',
      'Raise both arms straight overhead',
      'Square your hips toward the front',
      'Lift your chest and look forward',
    ],
    benefits: ['Strengthens legs', 'Opens hips and chest', 'Builds focus'],
  },
  {
    id: 'warrior_two',
    name: 'Warrior II',
    sanskritName: 'Virabhadrasana II',
    level: 'Beginner',
    duration: 45,
    description: 'A strong standing pose that increases stamina and opens the hips and chest.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1575052814086-f385e2e2ad1b?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4',
    instructions: [
      'Stand with feet wide apart (about 4 feet)',
      'Turn your right foot out 90 degrees',
      'Turn your left foot in slightly',
      'Extend arms out to the sides at shoulder height',
      'Bend your right knee to 90 degrees over ankle',
      'Keep your torso centered between your legs',
      'Gaze over your right fingertips',
      'Hold for 30-45 seconds, then switch sides',
    ],
    targetAngles: {
      leftElbow: 170,
      rightElbow: 170,
      leftKnee: 170,
      rightKnee: 90,
      leftShoulder: 90,
      rightShoulder: 90,
      leftHip: 170,
      rightHip: 100,
    },
    jointWeights: {
      leftElbow: 0.4, rightElbow: 0.4, leftKnee: 0.6, rightKnee: 1.0,
      leftShoulder: 0.9, rightShoulder: 0.9, leftHip: 0.7, rightHip: 0.8,
    },
    angleTolerance: {
      leftElbow: 15, rightElbow: 15, leftKnee: 15, rightKnee: 12,
      leftShoulder: 15, rightShoulder: 15, leftHip: 15, rightHip: 15,
    },
    cameraHint: 'front',
    cameraHintText: 'Place your phone standing up, facing you directly',
    setupInstructions: [
      'Stand with your feet wide apart',
      'Turn your right foot out 90 degrees',
      'Extend both arms out at shoulder height',
      'Bend your right knee over your ankle',
      'Look over your right hand',
    ],
    benefits: ['Builds leg strength', 'Improves balance', 'Opens groin'],
  },
  {
    id: 'tree_pose',
    name: 'Tree Pose',
    sanskritName: 'Vrksasana',
    level: 'Beginner',
    duration: 30,
    description: 'An elegant balancing pose that strengthens the legs and core while improving focus.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1510894347713-fc3ed6fdf539?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4',
    instructions: [
      'Begin in Mountain Pose',
      'Shift weight to your left foot',
      'Place right foot on inner left thigh (avoid the knee)',
      'Press foot and thigh against each other',
      'Bring hands to prayer position at heart center',
      'Or extend arms overhead like branches',
      'Fix gaze on a steady point ahead',
      'Hold for 30 seconds, then switch sides',
    ],
    targetAngles: {
      leftElbow: 170, // FIXED: arms overhead should be straight
      rightElbow: 170, // FIXED: arms overhead should be straight
      leftKnee: 175,
      rightKnee: 45,
      leftShoulder: 170,
      rightShoulder: 170,
      leftHip: 175,
      rightHip: 90,
    },
    jointWeights: {
      leftElbow: 0.5, rightElbow: 0.5, leftKnee: 0.9, rightKnee: 1.0,
      leftShoulder: 0.7, rightShoulder: 0.7, leftHip: 0.8, rightHip: 0.9,
    },
    angleTolerance: {
      leftElbow: 20, rightElbow: 20, leftKnee: 12, rightKnee: 12,
      leftShoulder: 20, rightShoulder: 20, leftHip: 15, rightHip: 15,
    },
    cameraHint: 'front',
    cameraHintText: 'Place your phone standing up, facing you directly',
    setupInstructions: [
      'Stand on your left foot',
      'Place your right foot on your inner left thigh',
      'Bring your hands together at your chest',
      'Find a steady point to focus on',
      'Keep your balance and breathe',
    ],
    benefits: ['Improves balance', 'Strengthens core', 'Builds concentration'],
  },
  {
    id: 'downward_dog',
    name: 'Downward-Facing Dog',
    sanskritName: 'Adho Mukha Śvānāsana',
    level: 'Beginner–Intermediate',
    duration: 60,
    category: 'Standing Inversion',
    primaryFocus: 'Spine Lengthening, Shoulder Stability, Hamstring Flexibility',
    description: 'A classic standing inversion pose that stretches the entire back body while building upper body strength.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1599901860904-17e6ed7083a0?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerFun.mp4',
    instructions: [
      'Stand on all fours in a tabletop position with hands shoulder-width apart',
      'Spread your fingers wide and press firmly into your palms',
      'Tuck your toes under and lift your knees off the floor',
      'Raise your hips upward and backward to form an inverted V shape',
      'Lengthen your spine and press your chest gently toward your thighs',
      'Relax your neck, letting your head hang naturally looking toward your feet',
      'Let your heels move toward the floor without forcing them',
      'Breathe steadily and hold for 20–60 seconds (5–10 breaths)',
    ],
    targetAngles: {
      leftElbow: 180,
      rightElbow: 180,
      leftKnee: 175,
      rightKnee: 175,
      leftShoulder: 175,
      rightShoulder: 175,
      leftHip: 120,
      rightHip: 120,
    },
    jointWeights: {
      leftShoulder: 1.0, rightShoulder: 1.0,
      leftHip: 1.0, rightHip: 1.0,
      leftElbow: 0.9, rightElbow: 0.9,
      leftKnee: 0.7, rightKnee: 0.7,
    },
    angleTolerance: {
      leftShoulder: 10, rightShoulder: 10,
      leftHip: 12, rightHip: 12,
      leftElbow: 10, rightElbow: 10,
      leftKnee: 15, rightKnee: 15,
    },
    customFeedback: {
      leftElbow: {
        tooSmall: { mild: 'Straighten both arms and press the floor away.', strong: 'Straighten both arms and press the floor away.' },
      },
      rightElbow: {
        tooSmall: { mild: 'Straighten both arms and press the floor away.', strong: 'Straighten both arms and press the floor away.' },
      },
      leftHip: {
        tooBig: { mild: 'Raise your hips toward the ceiling.', strong: 'Lift your hips higher and lengthen your spine.' },
        tooSmall: { mild: 'Shift your hips back to balance the pose.', strong: 'Shift your hips back to balance the pose.' },
      },
      rightHip: {
        tooBig: { mild: 'Raise your hips toward the ceiling.', strong: 'Lift your hips higher and lengthen your spine.' },
        tooSmall: { mild: 'Shift your hips back to balance the pose.', strong: 'Shift your hips back to balance the pose.' },
      },
      leftShoulder: {
        tooSmall: { mild: 'Push the floor away and broaden your upper back.', strong: 'Push the floor away and broaden your upper back.' },
      },
      rightShoulder: {
        tooSmall: { mild: 'Push the floor away and broaden your upper back.', strong: 'Push the floor away and broaden your upper back.' },
      },
      leftKnee: {
        tooSmall: { mild: 'Gently straighten your legs while keeping your spine long.', strong: 'Gently straighten your legs while keeping your spine long.' },
      },
      rightKnee: {
        tooSmall: { mild: 'Gently straighten your legs while keeping your spine long.', strong: 'Gently straighten your legs while keeping your spine long.' },
      },
    },
    cameraHint: 'side',
    cameraHintText: 'Place your phone to the side (or 45° diagonal) for best spine and hip visibility',
    setupInstructions: [
      'Stand on all fours in a tabletop position',
      'Place your hands shoulder-width apart and spread fingers wide',
      'Tuck your toes and lift your knees off the floor',
      'Raise your hips upward and backward into an inverted V',
      'Lengthen your spine and relax your neck',
    ],
    benefits: ['Spine lengthening', 'Shoulder stability & strength', 'Hamstring & calf flexibility', 'Calms the mind'],
  },
  {
    id: 'cobra_pose',
    name: 'Cobra Pose',
    sanskritName: 'Bhujangasana',
    level: 'Intermediate',
    duration: 30,
    description: 'A gentle backbend that strengthens the spine and opens the chest and shoulders.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1552196563-55cd4e45efb3?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerJoyrides.mp4',
    instructions: [
      'Lie face down with legs extended behind you',
      'Place hands under your shoulders',
      'Press the tops of your feet into the floor',
      'Inhale and lift your chest off the floor',
      'Keep your elbows slightly bent and close to body',
      'Press your pelvis into the floor',
      'Draw your shoulders back and down',
      'Hold for 15-30 seconds',
    ],
    targetAngles: {
      leftElbow: 130,
      rightElbow: 130,
      leftKnee: 175,
      rightKnee: 175,
      leftShoulder: 40,
      rightShoulder: 40,
      leftHip: 170,
      rightHip: 170,
    },
    jointWeights: {
      leftElbow: 0.8, rightElbow: 0.8, leftKnee: 0.4, rightKnee: 0.4,
      leftShoulder: 0.9, rightShoulder: 0.9, leftHip: 0.7, rightHip: 0.7,
    },
    angleTolerance: {
      leftElbow: 15, rightElbow: 15, leftKnee: 15, rightKnee: 15,
      leftShoulder: 20, rightShoulder: 20, leftHip: 15, rightHip: 15,
    },
    cameraHint: 'side',
    cameraHintText: 'Place your phone to the side for best results',
    setupInstructions: [
      'Lie face down on the floor',
      'Place your hands under your shoulders',
      'Press into your hands and lift your chest',
      'Keep your elbows slightly bent',
      'Draw your shoulders back',
    ],
    benefits: ['Strengthens spine', 'Opens chest', 'Relieves fatigue'],
  },
  {
    id: 'triangle_pose',
    name: 'Triangle Pose',
    sanskritName: 'Trikonasana',
    level: 'Intermediate',
    duration: 45,
    description: 'A standing pose that stretches and strengthens the thighs, knees, and ankles.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1573590330530-7ed413382451?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerMeltdowns.mp4',
    instructions: [
      'Stand with feet wide apart',
      'Turn your right foot out 90 degrees',
      'Extend arms parallel to the floor',
      'Reach your right hand toward your right foot',
      'Extend your left arm straight up',
      'Keep both legs straight',
      'Look up at your left hand',
      'Hold for 30-45 seconds, then switch sides',
    ],
    targetAngles: {
      leftElbow: 170,
      rightElbow: 170,
      leftKnee: 175,
      rightKnee: 175,
      leftShoulder: 170,
      rightShoulder: 90,
      leftHip: 170,
      rightHip: 90,
    },
    jointWeights: {
      leftElbow: 0.4, rightElbow: 0.4, leftKnee: 0.7, rightKnee: 0.7,
      leftShoulder: 0.9, rightShoulder: 0.9, leftHip: 0.8, rightHip: 1.0,
    },
    angleTolerance: {
      leftElbow: 15, rightElbow: 15, leftKnee: 15, rightKnee: 15,
      leftShoulder: 15, rightShoulder: 15, leftHip: 15, rightHip: 15,
    },
    cameraHint: 'front',
    cameraHintText: 'Place your phone standing up, facing you directly',
    setupInstructions: [
      'Stand with your feet wide apart',
      'Turn your right foot out 90 degrees',
      'Extend your arms out to the sides',
      'Reach down to your right shin with your right hand',
      'Extend your left arm straight up',
    ],
    benefits: ['Stretches legs', 'Reduces stress', 'Improves digestion'],
  },
  {
    id: 'chair_pose',
    name: 'Chair Pose',
    sanskritName: 'Utkatasana',
    level: 'Intermediate',
    duration: 30,
    description: 'A challenging standing pose that strengthens the ankles, thighs, calves, and spine.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1518611012118-696072aa579a?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/Sintel.mp4',
    instructions: [
      'Stand in Mountain Pose',
      'Inhale and raise your arms overhead',
      'Exhale and bend your knees as if sitting in a chair',
      'Keep your thighs as parallel to the floor as possible',
      'Keep your knees over your ankles',
      'Lean your torso slightly forward',
      'Keep your spine long',
      'Hold for 20-30 seconds',
    ],
    targetAngles: {
      leftElbow: 170,
      rightElbow: 170,
      leftKnee: 110,
      rightKnee: 110,
      leftShoulder: 170,
      rightShoulder: 170,
      leftHip: 100,
      rightHip: 100,
    },
    jointWeights: {
      leftElbow: 0.3, rightElbow: 0.3, leftKnee: 1.0, rightKnee: 1.0,
      leftShoulder: 0.7, rightShoulder: 0.7, leftHip: 0.9, rightHip: 0.9,
    },
    angleTolerance: {
      leftElbow: 20, rightElbow: 20, leftKnee: 12, rightKnee: 12,
      leftShoulder: 20, rightShoulder: 20, leftHip: 12, rightHip: 12,
    },
    cameraHint: 'front',
    cameraHintText: 'Place your phone standing up, facing you directly',
    setupInstructions: [
      'Stand with your feet together',
      'Raise your arms straight overhead',
      'Bend your knees as if sitting in a chair',
      'Keep your weight in your heels',
      'Keep your spine long and chest lifted',
    ],
    benefits: ['Strengthens thighs', 'Tones core', 'Builds endurance'],
  },
  {
    id: 'bridge_pose',
    name: 'Bridge Pose',
    sanskritName: 'Setu Bandhasana',
    level: 'Advanced',
    duration: 30,
    description: 'A rejuvenating backbend that opens the chest, strengthens the back, and calms the mind.',
    thumbnailUrl: 'https://images.unsplash.com/photo-1562088287-bde35a1ea917?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/SubaruOutbackOnStreetAndDirt.mp4',
    instructions: [
      'Lie on your back with knees bent, feet flat',
      'Place feet hip-width apart, close to sitting bones',
      'Press feet and arms into the floor',
      'Lift your hips toward the ceiling',
      'Clasp your hands beneath your back',
      'Roll your shoulders underneath',
      'Keep thighs parallel to each other',
      'Hold for 30-60 seconds',
    ],
    targetAngles: {
      leftElbow: 170,
      rightElbow: 170,
      leftKnee: 80,
      rightKnee: 80,
      leftShoulder: 10,
      rightShoulder: 10,
      leftHip: 160,
      rightHip: 160,
    },
    jointWeights: {
      leftElbow: 0.4, rightElbow: 0.4, leftKnee: 1.0, rightKnee: 1.0,
      leftShoulder: 0.5, rightShoulder: 0.5, leftHip: 0.9, rightHip: 0.9,
    },
    angleTolerance: {
      leftElbow: 20, rightElbow: 20, leftKnee: 15, rightKnee: 15,
      leftShoulder: 20, rightShoulder: 20, leftHip: 15, rightHip: 15,
    },
    cameraHint: 'side',
    cameraHintText: 'Place your phone to the side for best results',
    setupInstructions: [
      'Lie on your back with knees bent',
      'Place your feet flat, hip width apart',
      'Press your feet down and lift your hips',
      'Keep your thighs parallel',
      'Hold your hips up and breathe',
    ],
    benefits: ['Strengthens back', 'Opens chest', 'Reduces anxiety'],
  },
  {
    id: 'childs_pose',
    name: "Child's Pose",
    sanskritName: 'Balasana',
    level: 'Beginner',
    duration: 60,
    description: "A gentle resting pose that stretches the hips, thighs, and ankles while calming the mind.",
    thumbnailUrl: 'https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=400',
    videoUrl: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/TearsOfSteel.mp4',
    instructions: [
      'Kneel on the floor with toes together',
      'Sit on your heels',
      'Separate your knees about hip-width apart',
      'Exhale and fold forward, resting torso on thighs',
      'Extend your arms forward on the floor',
      'Rest your forehead on the mat',
      'Let your whole body relax',
      'Hold for 1-3 minutes',
    ],
    targetAngles: {
      leftElbow: 170,
      rightElbow: 170,
      leftKnee: 30,
      rightKnee: 30,
      leftShoulder: 170,
      rightShoulder: 170,
      leftHip: 40,
      rightHip: 40,
    },
    jointWeights: {
      leftElbow: 0.3, rightElbow: 0.3, leftKnee: 0.9, rightKnee: 0.9,
      leftShoulder: 0.6, rightShoulder: 0.6, leftHip: 1.0, rightHip: 1.0,
    },
    angleTolerance: {
      leftElbow: 20, rightElbow: 20, leftKnee: 20, rightKnee: 20,
      leftShoulder: 20, rightShoulder: 20, leftHip: 20, rightHip: 20,
    },
    cameraHint: 'side',
    cameraHintText: 'Place your phone to the side for best results',
    setupInstructions: [
      'Kneel on the floor',
      'Sit back on your heels',
      'Fold forward and rest your forehead down',
      'Extend your arms forward on the floor',
      'Relax your whole body',
    ],
    benefits: ['Relieves stress', 'Stretches hips', 'Calms the brain'],
  },
];

export const getDifficultyColor = (level) => {
  switch (level) {
    case 'Beginner': return '#00E676';
    case 'Intermediate': return '#FFD600';
    case 'Advanced': return '#FF5252';
    default: return '#B0BEC5';
  }
};

export const getPoseById = (id) => {
  return YOGA_POSES.find(pose => pose.id === id);
};

export const getPosesByLevel = (level) => {
  if (level === 'All') return YOGA_POSES;
  return YOGA_POSES.filter(pose => pose.level === level);
};
